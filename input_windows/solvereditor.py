...
from tkinter             import Frame, Label, Entry, Checkbutton, OptionMenu, StringVar, IntVar, DoubleVar, Button, messagebox
...
class SolverEditor:
    ...
    def __init__(self, master, system, ptype, ptotal, tvariable, delt, delz, players, depoption, depgrid):
        """Constructor method."""
        ...
        self.depoption      = StringVar(value = depoption)
    ...
    def make_widgets(self):
        ...
        self.depoptionlabel   = Label(self.tframe, text = 'Deposition option:', width = 15)
        self.depoptionwidget  = Label(self.tframe, text = 'User-defined', width = 15)
        self.depoptionwidget.config(width =15)
        self.depgridlabel     = Label(self.tframe, text = 'Deposition grid steps:',       width = 15)
        self.depgridwidget    = Label(self.tframe, textvariable = self.depgrid,    width = 10, justify = 'center')
        self.depgridentry     = Entry(self.tframe, textvariable = self.depgrid,    width = 10, justify = 'center')
        ...
        if self.dep == 2:
            self.depoptionlabel.grid(   row =  row, column = 0, sticky = 'E',  padx = 2, columnspan = 2)
            self.depoptionwidget.grid(  row =  row, column = 2, sticky = 'W',  padx = 2, columnspan = 2)
            self.depgridlabel.grid(     row =  row, column = 4, sticky = 'E',  padx = 2, columnspan = 2)
            row = row + 1
        ...
        try:
            ...
            self.depgridwidget.grid_forget()
            self.depgridentry.grid_forget()
        except: pass
        ...
        if self.dep == 2:
            if self.depoption.get() == self.depoptions[2]:  self.depgridentry.grid( row =  4, column = 6, sticky = 'E', padx = 2)
            else:                                           self.depgridwidget.grid(row =  4, column = 6, sticky = 'E', padx = 2)
    ...
    def updatesolverparameters(self):
        ...
        if self.dep == 1:
            ...
            if self.depoption.get() == self.depoptions[1]:   self.depgrid.set(round_to_n(self.delz[0]/self.delz[0], 3))
            elif self.depoption.get() == self.depoptions[0]: self.depgrid.set(round_to_n(self.delz[0]/self.delt.get()/self.Vdep, 3))
    ...
    def updatesolvereditor(self, event = None):
        ...
        row = 4
        if self.dep == 2:
            if self.depoption.get() == self.depoptions[2]:  self.depgridentry.grid( row =  row, column = 6, sticky = 'E', padx = 2)
            else:                                           self.depgridwidget.grid(row =  row, column = 6, sticky = 'E', padx = 2)
            row = row + 1

...
from tkinter             import Frame, Label, Entry, OptionMenu, Button, Text, DoubleVar, StringVar, IntVar, messagebox, font
...
class ReactionEditor:
    ...
    def click_model(self, event = None):
        ...
        if len(self.reactants) == 0:
            messagebox.showerror(title = self.system.version, message = 'Please firstly input the information of reactants.')
            self.model.set('Fundamental')
            self.focusbutton     = self.okbutton
            self.master.tk.lift()
            
        elif check == 1:
            messagebox.showerror(title = self.system.version, message = 'At least one chemical is replicated in the reactant list, please check')
            self.model.set('Fundamental')
            self.focusbutton     = self.okbutton
            self.master.tk.lift()
    ...
    def updaterateequation(self):
        ...
        mainfont      = font.Font(font = self.formulatype)
        subfont   = font.Font(font = self.subfont)
        superfont = font.Font(font = self.superfont)

        if self.model.get() == 'Fundamental':

            rate_len  = mainfont.measure(u'r = \u03BB')
            for reactant in self.reactants:
                rate_len = rate_len + mainfont.measure('C')
    ...
    def reaction_error(self):
        ...
        messagebox.showerror(title = self.version, message = 'At least one chemical has been replicated in the reaction!')
        self.focusbutton = self.okbutton
        self.master.tk.lift()

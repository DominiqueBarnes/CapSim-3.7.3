...
from tkinter              import Tk, Toplevel, Canvas, Frame, Label, Entry, Text, Button, Scrollbar, OptionMenu, StringVar, \
                                DoubleVar, IntVar, FLAT, RAISED, Checkbutton, messagebox
...
def get_summary(system, database, materials):
    ...
    if root.main.get() == 0:
        check = 1
        try:
            pickle.dump(root.window.system, open(Filepath + '/temp_test_file.cpsm', 'wb'))
        except:
            check = 0

        if check == 1:
            pickle.dump(root.window.system, open(Filepath + '/input_cpsm_files/%s.cpsm' %system.cpsmfilename, 'wb'))
            ...
            if report_check == 0:
                try:
                    pickle.dump(root.window.system.chemicals, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                except:
                    report_check = 1
                    messagebox.showwarning('Administrator Error', 'It appears you have a problem related to chemical properties in saving your input file. Please re-try it.')

            if report_check == 0:
                try:
                    pickle.dump(root.window.system.matrices, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                except:
                    report_check = 1
                    messagebox.showwarning('Administrator Error', 'It appears you have a problem related to material properties in saving your input file. Please re-try it.')

            if report_check == 0:
                try:
                    pickle.dump(root.window.system.sorptions, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                except:
                    report_check = 1
                    messagebox.showwarning('Administrator Error', 'It appears you have a problem related to sorption properties in saving your input file. Please re-try it.')

            if report_check == 0:
                try:
                    pickle.dump(root.window.system.layers, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                except:
                    report_check = 1
                    messagebox.showwarning('Administrator Error', 'It appears you have a problem related to layer properties in saving your input file. Please re-try it.')

            if report_check == 0:
                try:
                    pickle.dump(root.window.system.reactions, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                except:
                    report_check = 1
                    messagebox.showwarning('Administrator Error', 'It appears you have a problem related to reaction properties in saving your input file. Please re-try it.')

            if report_check == 0:
                try:
                    pickle.dump(root.window.system.coefficients, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                except:
                    report_check = 1
                    messagebox.showwarning('Administrator Error', 'It appears you have a problem related to reaction coefficients in saving your input file. Please re-try it.')

            if report_check == 0:
                try:
                    pickle.dump(root.window.system.ICs, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                    pickle.dump(root.window.system.BCs, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                    pickle.dump(root.window.system.SolidICs, open(Filepath + '/temp_test_file.cpsm', 'wb'))
                except:
                    report_check = 1
                    messagebox.showwarning('Administrator Error', 'It appears you have a problem related to auxiliary conditions in saving your input file. Please re-try it.')

            if report_check == 0:
                messagebox.showwarning('Administrator Error', 'It appears you have a problem related to system parameters or solver options in saving your input file. Please re-try it.')

...
class LayerConditions:
    ...
    def updateconditions(self, event = None, *args):
        ...
        if self.topBCtype.get() == self.topBCtypes[2]:
            ...
            if self.CSTR_flag == 0:
                ...
                messagebox.showerror(title = 'Model Error', message = 'The water body CSTR model is not applicable for systems with deposition layer governing by non-linear equilibrium isotherm or transient sorption kinetics')
                ...
                if self.top.window.bltype.get() == 'River':
                    ...
                    for chemical in self.chemicals:
                        if chemical.soluable == 1:
                            ...
                            except: messagebox.showerror(title = 'Math Error', message = 'The parameters you have entered produce a math error.  Please change the parameters and recalculate.')
                else:
    ...
    def warning(self):
        ...
        messagebox.showerror(title = self.version, message = 'The retention time must be positive values')
        self.focusbutton = None
        self.master.tk.lift()

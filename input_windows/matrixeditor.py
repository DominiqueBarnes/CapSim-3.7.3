...
from tkinter             import Frame, Label, Entry, OptionMenu, Button, Text, \
                                DoubleVar, StringVar, IntVar, messagebox
...
class MatrixEditor:
    ...
    def __init__(self, master, system, matrix, matrices, materials, editflag, newflag):
        ...
        self.matrices_list = sorted(self.matrices_list)
    ...
    def OK(self, event = None):
        ...
        if self.master.window.top is not None: self.master.open_toplevel()
             ...
             messagebox.showinfo(self.version, 'The porosity of a solid can not be larger than 1 or smaller than 0')
             self.e.set(0.5)
        elif self.rho.get() < 0:
            messagebox.showinfo(self.version, 'The bulk density of a solid can not be negative')
            self.e.set(1.0)
        elif self.foc.get() > 1 or self.foc.get() < 0: 
            messagebox.showinfo(self.version, 'The organic carbon fraction of a solid can not be larger than 1 or smaller than 0')
            self.e.set(1.0)
    ...
    def matrix_error(self):
        ...
        messagebox.showerror(title = self.version, message = 'This solid material has already been added to the database!')
        self.focusbutton = self.okbutton
        self.master.tk.lift()
...
class MixtureEditor:
    ...
    def matrix_error(self):
        ...
        messagebox.showerror(title = self.version, message = 'This matrix has already been added to the matrix list!')
        self.focusbutton = self.okbutton
        self.master.tk.lift()

    def empty_component(self):

        messagebox.showerror(title = self.version, message = 'The mixture must consist at least two components')
        self.focusbutton = self.okbutton
        self.master.tk.lift()
        
    def fraction_error(self):

        messagebox.showinfo(self.version, 'The sum of the weight fraction is not 1, please check')
        self.focusbutton = self.okbutton
        self.master.tk.lift()

    def rho_error(self):

        messagebox.showinfo(self.version, 'The bulk density of a component must be a positive value')
        self.focusbutton = self.okbutton
        self.master.tk.lift()

    def replicate_error(self):

        messagebox.showinfo(self.version, 'At least one component is replicated in the mixture')
        self.focusbutton = self.okbutton
        self.master.tk.lift()

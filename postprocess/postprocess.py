...
import matplotlib.pyplot as plt, math, sys, os, codecs, winreg as wreg
...
from tkinter             import Tk, Toplevel, Canvas, Frame, Label, Entry, Text, Button, Scrollbar, OptionMenu, StringVar, DoubleVar, IntVar, FLAT, RAISED, \
                                Checkbutton, messagebox, filedialog
...
class Postprocess:
    ...
    def editplot(self, event = None):
        ...
        if self.top is None:
            ...
            messagebox.showerror(title = self.system.version, message = 'Please close the existing parameter input window first.')
            self.top.tk.focus()
    ...
    def savegraph(self, event = None):
        ...
        self.graphname = filedialog.asksaveasfilename(initialdir=self.graphpath,  title='Please define the directory to save the graph', filetypes = [('PNG files', '*.png')])
    ...
    def export_data(self, event = None):
        ...
        if self.top is None:
            ...
            messagebox.showerror(title = self.system.version, message = 'Please close the existing result export window first.')
            self.top.tk.focus()
...
class GraphEditor:
    ...
    def warning(self):
        ...
        messagebox.showerror(title = self.version, message = 'The input depth/time is out of range, please correct')
...
class FigureEditor:
    ...
    def warning(self):
        ...
        messagebox.showerror(title = self.version, message = 'The minimum axis must be smaller than the maximum axis value')
        self.focusbutton = None
        self.master.tk.lift()
...
class ResultExporter:
    ...
    def browse_dir(self, event = None):
        ...
        self.filename = filedialog.asksaveasfilename(initialdir=self.filepath.get(), initialfile = self.name.get(), title='Please define the directory to save the file', filetypes = filetype)

...
from tkinter             import Frame, Label, Entry, OptionMenu, Button, Tk, Text, StringVar, DoubleVar, IntVar, messagebox, filedialog
...
class Database:
    ...
    def importsoliddata(self):
        ...
        Filepath = filedialog.askopenfilename(initialdir = UserPath)
        ...
        if Filepath != '':
            ...
            if self.top is None:
                ...
                if self.top.window.cancelflag == 0:
                    ...
                    if error_check == 1: messagebox.showerror(title = self.system.version, message = error_message)
    ...
    def exit(self, event = None):
        ...
        if messagebox.askyesno(self.version, 'Do you really want to exit CapSim?') == 1: sys.exit()
...
def make_soliddatabase(path):
    ...
    pickle.dump(soliddatabase, open(path + r'/database/capsim_soliddatabase', 'wb'))
    
    return open(path + r'/database/capsim_soliddatabase', 'rb')

...
class DatabaseEditor:
    ...
    def edit_formula (self, event = None):
        ...
        if self.top is None:
            ...
            messagebox.showerror(title = self.version, message = 'Please close the existing parameter input window first.')
            self.top.tk.focus()
    ...
    def save(self, event = None):
        ...
        if Koc_check == 1:  messagebox.showinfo(self.version, 'The units of the organic carbon partition coefficient are log (L/kg)!  The value will be set the maximum of 20.')
        if Kdoc_check == 1: messagebox.showinfo(self.version, 'The units of the dissolved organic carbon partition coefficient are log (L/kg)!  The value will be set the maximum of 20.')
        ...
        if sum(name_check) >= 1:   messagebox.showerror(title = self.version, message = 'The input chemical already exists!')
        elif sum(temp_check) >= 1: messagebox.showerror(title = self.version, message = 'The input temperature already exists!')
...
class DatabaseImporter:
    ...
    def __init__(self, master, system, database_imported):
        ...
        self.tframe        = Frame(master.tframe)
        self.bframe        = Frame(master.bframe)
        self.tkfont        = font.Font(font = system.fonttype)

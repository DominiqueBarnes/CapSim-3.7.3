...
try:
    ...
    Filepath = filedialog.askdirectory(initialdir=UserPath+r'\Documents', parent = rootinit, title='To initialize CapSim, please select the directory for input and output files')
    if Filepath is not '':
...
while(1):
    ...
    try:
        data = open(Filepath + r'/database/capsim3_chemicaldatabase', 'rb')
        database = pickle.load(data)
        data.close()
    except:
        try:
            data = open(Filepath + r'/database/capsim_chemicaldatabase', 'rb')
            database = pickle.load(data)
            data.close()
            database = get_updateddatabase(database)
            pickle.dump(database, open(Filepath + r'/database/capsim3_chemicaldatabase', 'wb'))
        except:
            data = make_database(Filepath)
            database = pickle.load(data)
            data.close()

    try:    materialdata = open(Filepath + r'/database/capsim_soliddatabase', 'rb')
    ...
    if option == 5:
        ...
        while(1):
            data = open(Filepath + r'/database/capsim_soliddatabase', 'rb')
            database = pickle.load(data)
            data.close()

            database, main = edit_soliddatabase(database, system)

            pickle.dump(database, open(Filepath + r'/database/capsim_soliddatabase', 'wb'))
            if main == 1: break
